...
| 애니메이션 단계 | `index.html` PATH_ANIM `txt` | 5건 (558 체크리스트) | `주기장별절차/*.json` 은 툴 산출물이라 앱이 읽지 않음 |
...
□ UI 문구
  ...
  - 대상: 헤더·홈·주기장 상세·라이트박스·경로이탈 경보·GPS 상태·오프라인 안내·도면 애니메이션 단계. 이용집계·기준점 측정 모드(관리자 전용)는 제외
